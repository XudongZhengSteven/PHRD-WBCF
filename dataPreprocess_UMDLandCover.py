...
def ExtractLCForEachBasinGrids(grid_shp):
    ...
    umd_path = [os.path.join(umd_landcover_qdeg_path, f"umd_landcover_qd_c0{i}.asc") for i in range(10)]
    umd_path.extend(os.path.join(umd_landcover_qdeg_path, f"umd_landcover_qd_c{i}.asc") for i in range(10, 15))
    umd_path.sort()
    umd_landcover_Cv_all = [np.loadtxt(path, skiprows=6) for path in umd_path]  # 0-14 class

    major_umd_landcover_qdeg_path = os.path.join(umd_landcover_qdeg_path, "umd_landcover_class_qd.asc")
    major_umd_landcover_qdeg = np.loadtxt(major_umd_landcover_qdeg_path, skiprows=6)

    # umd lat lon
    umd_lat = np.arange(89.875, -90.125, -0.25)  # -90.125 = -89.875 - 0.25 (last not contain)
    ...
    searched_grids_index = search_grids_equal(dst_lat=grids_lat, dst_lon=grids_lon, src_lat=umd_lat, src_lon=umd_lon)
    ...
    for i in tqdm(grid_shp.index, colour="green", desc="loop for each grid to extract LC"):
        ...
        for j in range(len(umd_landcover_Cv_all)):
            ...
            umd_landcover_classification_grid.append(
                umd_landcover_Cv[searched_grid_index[0][0],
        ...
        major_umd_landcover_classification_grids.append(
            major_umd_landcover_qdeg[searched_grid_index[0][0],
                                     searched_grid_index[1][0]])

    grid_shp["umd_landcover_classification"] = umd_landcover_classification_grids
    grid_shp["major_umd_landcover_classification_grids"] = major_umd_landcover_classification_grids
