...
def combineGLDAS(var_name="CanopInt_tavg"):
    ...
    src_fnames = np.array([n for n in os.listdir(home) if n.endswith(suffix)], dtype=str)
    ...
    for j in range(len(src_fpaths_list)):
        ...
        for i in tqdm(range(len(src_paths_)), desc="loop for files to combineGLDAS", colour="green"):
            # general
        ...
        dst_fpath = os.path.join("H:/data/GLDAS/GLDAS_025_daily_1948_2014_global_CLSM/", dst_fname)
        dataset_var.to_netcdf(dst_fpath)
        
        dataset_var.close()
    

def ExtractGLDAS(grid_shp, period=["19800101", "20101231"], var_name="CanopInt_tavg"):
    # general
    ...
    for i in range(len(grid_shp.index)):
        ...
        for j in range(len(src_datasets)):
            ...
            date_period_index = np.where((src_date >= datetime.strptime(period[0], "%Y%m%d")) & (src_date <= datetime.strptime(period[1], "%Y%m%d")))[0]
