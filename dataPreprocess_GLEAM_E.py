...
def ExtractGLEAM_E_daily(grid_shp, period=["19800101", "20101231"], var_name="E"):
    # period
    period_date = [datetime.strptime(period[0], "%Y%m%d"), datetime.strptime(period[1], "%Y%m%d")]
    ...
    src_dirs = [d for d in os.listdir(home) if (os.path.isdir(os.path.join(home, d)) and int(d) >= period_year[0] and int(d) <= period_year[1])]
    src_fnames = [f"{var_name}_{d}_GLEAM_{version}{suffix}" for d in src_dirs]
    src_paths = [os.path.join(home, d, f"{var_name}_{d}_GLEAM_{version}{suffix}") for d in src_dirs]
    ...
    with xr.open_dataset(src_paths[0]) as dataset:
        ...
        searched_grids_index = search_grids.search_grids_equal(dst_lat, dst_lon, src_lat, src_lon)

    # set date and search date_period_index
    grid_df_list = []
    for i in tqdm(range(len(grid_shp.index)), desc="loop for grid to extract gleam_e_daily", colour="green"):
        ...
        for j in range(len(src_paths)):
            ...
            with xr.open_dataset(src_path) as src_dataset:
                ...
                date_period_index = np.where((date >= datetime.strptime(period[0], "%Y%m%d")) & (date <= datetime.strptime(period[1], "%Y%m%d")))[0]
                
                # loop for searched grids to cal grid data
                var_data = src_dataset[var_name].isel(time=date_period_index, lat=xr.DataArray(searched_grid_lat_index, dims="z"), lon=xr.DataArray(searched_grid_lon_index, dims="z"))
